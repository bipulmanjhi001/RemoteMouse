...
import org.pierre.remotedroid.protocol.tcp.PRemoteDroidConnection;
		...
		PRemoteDroidConnection c = null;
			...
			try
			{
				...
				try
				{
					...
					Log.e("TEsting Preote droid>>>", "try called");
					String password = co.getPassword();
					Log.e("TEsting Preote droid>>>",""+password);
					this.sendAction(new AuthentificationAction(password));
					
					while (true)
					{
						Log.e("TEsting Preote droid>>>","inside while"+c.receiveAction());

							PRemoteDroidAction action = c.receiveAction();
						Log.e("TEsting Preote droid>>>",""+action);
